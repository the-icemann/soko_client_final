...
export function confidenceMeta(c: string): { label: string; color: string } {
  switch (c?.toLowerCase()) {
    case "high":   return { label: "High",   color: "text-green-600"  };
    case "medium": return { label: "Medium", color: "text-yellow-600" };
    default:       return { label: "Low",    color: "text-red-500"    };
  }
}
